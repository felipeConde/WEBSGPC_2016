...
Public Class DAO_Dashboard
    ...
    Function GetTop10(pTipo As String, vencimento As String, grupo As String, area As String, area_interna As String, codigousuario As String) As DataTable
        ...
        sql += " select sum(nvl(p1.total_gasto,p1.valor_cdr))+ nvl(max(p1.rateio),0)  gasto, to_char(p3.dt_vencimento, 'MM/YYYY') data, "
        If pTipo = "2" Then
            sql += "p1.rml_numero_a usuario"
        Else
            sql += " nvl(u.nome_usuario,p1.rml_numero_a) usuario"
        End If
        ...
        'parcelas
        sql += " union all "
        sql += " select nvl(max(nvl(pa.parcela,0)),0)  gasto, to_char(p3.dt_vencimento, 'MM/YYYY') data,nvl(u.nome_usuario,p1.rml_numero_a) usuario"

        'sql += " , p1.grp_codigo as codigo  "
        sql += " from CDRS_CELULAR_ANALITICO_MV p1, faturas_arquivos p2, faturas p3 , faturas_tipo p4, grupos p5,usuarios u ,V_LINHAS_PARCELAS_CUSTOS3  pa "
        sql += " where p1.codigo_conta=p2.codigo_conta and p3.codigo_fatura=p2.codigo_fatura  "
        sql += " and p3.codigo_tipo=p4.codigo_tipo(+) and p1.grp_codigo=p5.codigo(+) and p1.codigo_usuario=u.codigo(+) "
        sql += "  and p1.codigo_usuario=pa.codigo_usuario(+) and p1.rml_numero_a=pa.num_linha(+) and to_char(p3.dt_vencimento,'MM/YYYY')=pa.data "
        ...
        'CUSTO FIXO
        sql += " union all"
        sql += " select nvl(max(pa.custo_fixo),0)  gasto, to_char(p3.dt_vencimento, 'MM/YYYY') data,nvl(u.nome_usuario,p1.rml_numero_a) usuario"
        ...
        sql += " ) where gasto>0 group by data, usuario "
    ...
    Public Function CarregaAreas(codigoUsuario As String) As List(Of AppGeneric)
        ...
        If Not DALCGestor.AcessoAdmin(codigoUsuario) Then
            ...
            admin += "      and to_char(u.codigo) like cat.codigo_grupo||'%' ) " & vbNewLine
        End If
    ...
    Function GetServicosMes(pTipo As String, vencimento As String, grupo As String, area As String, area_interna As String, codigousuario As String) As DataTable
        ...
        sql += " from (select case when p3.codigo_tipo=1 then 'VOZ' else replace(nvl(p4.nome_configuracao,'VOZ'),'DEFAULT','VOZ') end tarifa, "
        ...
        sql += " group by to_char(p3.dt_vencimento, 'MM/YYYY'),p3.codigo_tipo,case when p3.codigo_tipo=1 then 'VOZ' else replace(nvl(p4.nome_configuracao,'VOZ'),'DEFAULT','VOZ') end "
        sql += " union "
    ...
    Public Function getUsuariosPerfis(vencimento As String, grupo As String, area As String, area_interna As String, codigousuario As String, Optional nivel As String = "") As DataTable
        ...
        If Not DALCGestor.AcessoAdmin(codigousuario) Then
            ...
            sql += "      and to_char(u.codigo) like cat.codigo_grupo||'%' ) " & vbNewLine
        End If
        sql += " )"
        ...
        If Not DALCGestor.AcessoAdmin(codigousuario) Then
            ...
            sql += "   select 0 from categoria_usuario cat" & vbNewLine
            sql += "     where cat.codigo_usuario=" + codigousuario & vbNewLine
            sql += "      and to_char(g.codigo) like cat.codigo_grupo||'%' ) " & vbNewLine
        End If
        sql += " )"
        sql += " group by codigo,nome_usuario"
        sql += " having count(*)>1 "
        ...
        If Not DALCGestor.AcessoAdmin(codigousuario) Then
            ...
            sql += "   select 0 from categoria_usuario cat" & vbNewLine
            sql += "     where cat.codigo_usuario=" + codigousuario & vbNewLine
            sql += "      and to_char(u.grp_codigo) like cat.codigo_grupo||'%' ) " & vbNewLine
        End If
        sql += " )"
        sql += " group by codigo,nome_usuario"
        sql += " having count(*)=1 "
    ...
    Public Function getLinhasPerfil(vencimento As String, grupo As String, area As String, area_interna As String, codigousuario As String, Optional nivel As String = "") As DataTable
        ...
        If Not DALCGestor.AcessoAdmin(codigousuario) Then
            ...
            sql += "   select 0 from categoria_usuario cat" & vbNewLine
            sql += "     where cat.codigo_usuario=" + codigousuario & vbNewLine
            sql += "      and to_char(gi.grupo) like cat.codigo_grupo||'%' ) " & vbNewLine
        End If
...
#Region "Ramal"
    ...
    Function GetTop10Ramal(pTipo As String, vencimento As String, grupo As String, area As String, area_interna As String, codigousuario As String) As DataTable
        ...
        sql += " select usuario, gasto,data "
        sql += " ,RANK() OVER (ORDER BY gasto desc)ordem "
        sql += " from( select usuario,sum(gasto) gasto, data  from( select usuario,sum(gasto) gasto, data  from( "
        'sql += " select sum(nvl(p1.total_gasto,p1.valor_cdr)) + nvl(max(p1.rateio),0) gasto, to_char(p3.dt_vencimento, 'MM/YYYY') data,nvl(u.nome_usuario,p1.rml_numero_a) usuario"
        'sql += " select sum(nvl(p1.total_gasto,p1.valor_cdr))+ nvl(max(p1.parcela), 0)  + (nvl(max(p1.custo_fixo), 0) )+ nvl(max(p1.rateio),0)  gasto, to_char(p3.dt_vencimento, 'MM/YYYY') data,nvl(u.nome_usuario,p1.rml_numero_a) usuario"
        sql += " select sum(nvl(p1.gasto,0)) gasto, p1.data,nvl(u.nome_usuario,p1.ramal) usuario"

        'sql += " , p1.grp_codigo as codigo  "
        sql += " from v_tarifacao2 p1, grupos p5,usuarios u  "
        sql += " where   "
        ...
        'aparelho
        sql += " union"
        sql += " select usuario, nvl(sum(gasto),0) gasto, data"
        sql += " from(select usuario, sum(p100.custo_ramal)  gasto, data"
        sql += " from (select p1.data data,p1.ramal,nvl(p1.usuario,p1.ramal)usuario,p1.custo_ramal "
        ...
        sql += " group by p1.ramal,p1.usuario, p1.custo_ramal "
        sql += " ,p1.data"
        sql += "  )p100, ramais r "
        ...
        sql += " from (select p1.data data,p1.ramal,nvl(p1.usuario,p1.ramal)usuario ,p1.custo_servico "
        ...
        sql += "  p1.data,  p1.ramal,p1.usuario,p1.custo_servico   "
        sql += "  )p100, ramais r "
        sql += " where p100.ramal=r.numero_a(+) "
        ...
        ' Session("exibir_todos") = False Then
        sql += " where ordem <=10 And rownum<=10 and gasto>0 order by ordem"
        'End If
