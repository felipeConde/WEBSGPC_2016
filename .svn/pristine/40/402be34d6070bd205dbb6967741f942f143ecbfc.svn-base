...
Public Class DAO_Commons
    ...
    Public Function GetGridRowCount() As String

        Dim rowCount As String = 5

        Dim sql As String = "select upper(t.valor_parametro)valor_parametro from PARAMETROS_SGPC t where t.nome_parametro='GRID_QTD_REGISTROS'"
        Dim dt As DataTable = myDataTable(sql)

        If dt.Rows.Count > 0 Then
            If dt.Rows(0).Item("valor_parametro") <> "" Then

                rowCount = dt.Rows(0).Item("valor_parametro")
