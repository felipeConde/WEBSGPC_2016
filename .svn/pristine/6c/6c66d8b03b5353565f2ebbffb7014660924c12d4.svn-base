...
Partial Class uc_graficoUsuarioMedia
    ...
    Function getQueryTotalGeralRamal(Optional pdata As String = "") As String
        ...
        If ViewState("codigo_usuario") <> "" And ViewState("codigo_usuario") > 0 Then
            sql += " union "
            sql += " select tarifa,round(sum(gasto)/sum(chamadas),2) GASTO, to_char(DATA)DATA FROM ( select 'MÉDIA' tarifa,  sum(nvl(p1.gasto,0)) gasto,sum(p1.chamadas)chamadas,P1.ramal,p1.data  "
            ...
            sql += " union all"
            sql += " select tarifa, sum(gasto) gasto,1 chamadas,ramal, data"
            sql += " from(select tarifa, p100.custo_ramal gasto, data,ramal"
            sql += " from (select 'MÉDIA' tarifa,p1.data data,p1.ramal,p1.custo_ramal "
            ...
            sql += " and to_date(p1.data,'MM/YYYY')>= to_date(to_char(add_months(to_date(to_char(sysdate,'MM/YYYY'),'MM/YYYY'),-11),'MM/YYYY'),'MM/YYYY') "
            sql += " and to_date(p1.data,'MM/YYYY')<= to_date(to_char(add_months(to_date(to_char(sysdate,'MM/YYYY'),'MM/YYYY'),0),'MM/YYYY'),'MM/YYYY')  " & vbNewLine
            sql += " group by  p1.tarifa, p1.data,  p1.ramal,p1.custo_ramal "
            sql += "  )p100, ramais r "
            sql += " where p100.ramal=r.numero_a(+) "
            sql += "   group by tarifa,  data,p100.custo_ramal,ramal "
            sql += " )group by tarifa,  data,ramal"

            'custo gestao
            sql += " union all"
            sql += " select tarifa, sum(gasto) gasto,1 chamadas,ramal, data"
            sql += " from(select tarifa, p100.custo_servico gasto,ramal, data"
            sql += " from (select 'MÉDIA' tarifa,p1.data data,p1.ramal,p1.custo_servico "
            ...
            sql += " and to_date(p1.data,'MM/YYYY')>= to_date(to_char(add_months(to_date(to_char(sysdate,'MM/YYYY'),'MM/YYYY'),-11),'MM/YYYY'),'MM/YYYY') "
            sql += " and to_date(p1.data,'MM/YYYY')<= to_date(to_char(add_months(to_date(to_char(sysdate,'MM/YYYY'),'MM/YYYY'),0),'MM/YYYY'),'MM/YYYY')  " & vbNewLine
            sql += " group by  p1.tarifa, p1.data,  p1.ramal ,p1.custo_servico"
            sql += "  )p100, ramais r "
            sql += " where p100.ramal=r.numero_a(+) "
            sql += "   group by tarifa,  data,p100.custo_servico,ramal "
            sql += " )group by tarifa,  data,ramal"
