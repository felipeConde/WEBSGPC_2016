<%@ Page Title="" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="GastoUsuarioRamal.aspx.vb" Inherits="GastoUsuarioRamal" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <div id="myApp" ng-app="SGPCAPP" ng-controller="usuarioRamaisCtrl">
        ...
        <div class="block-header">
         ...
         <input type="text" id="txtCodigoUsuario" ng-model="codigousuario" ng-init="codigousuario='<%=Session("codigousuario") %>'" style="display:none"/>
                <ul class="tab-nav" role="tablist" data-tab-color="amber">
                                        <li id="exibe_movel"  runat="server"><a href='GastoUsuario.aspx?mostraArea=<%= ViewState("mostraArea")  %> '>Moveis</a></li>
                                        <li class="active"><a href='#' role="tab" data-toggle="tab" aria-expanded="true">Ramais</a></li>                                        
                                    </ul>
                    ...
                    <div class="row">
                        ...
                        <button type="button" class="btn btn-primary dropdown-toggle waves-effect" data-toggle="dropdown" aria-expanded="false" style="margin-right: 10px;">
                            ...
                            <ul class="dropdown-menu" role="menu"  style="left:15px">
                                            <li><a href="gestaoRel_ConsumoRamaisResult.aspx?tipo=HTML&celular={{x.LINHA}}&mes={{ dtvencimento.Valor | limitTo : 2 : 0}}&ano={{ dtvencimento.Valor | limitTo : 4 : 3}}" target="_blank"><i class="glyphicon glyphicon-file"></i>HTML</a></li>
                                            <li><a href="gestaoRel_ConsumoRamaisResult.aspx?TIPO=EXCEL&celular={{x.LINHA}}&mes={{ dtvencimento.Valor | limitTo : 2 : 0}}&ano={{ dtvencimento.Valor | limitTo : 4 : 3}}" target="_blank"><i class="zmdi zmdi-grid zmdi-hc-fw"></i>EXCEL</a></li>
                           </ul>
    ...
    <div id="divGastos" ng-show="mostraGastos">
       ...
       <div class="card">
        ...
        <div class="card-header">   
            ...
            <div class="col-sm-3 m-b-15" >
              <select ng-model="dtvencimento" ng-options="x.Descricao for x in listVencimentos track by x.Valor"  data-placeholder="Selecione"  id="cmbVencimento"  style="width:130px; padding-left:5px;"  ng-change="getResumoGastoUsuario()" >                 
              </select>
                       ...
                       <table class="table table-striped">
                           <thead>
                               <tr>
                                   ...
                                   <th>CHAMADAS</th>
                                   <th>MINUTAGEM</th> 
                                   <% If MostraCreditoRamal %>
                                   <th>CRÉDITO MENSAL</th>  
                                   <% end if %>
                                   <th>MODELO</th>   
                                   <th>SERVIÇO</th>                                            
                                   <th>APARELHO</th>                                
                           ...
                           <tbody>
                               <tr ng-repeat="x in resumo">
                                   <td><a href="GestaoRel_ExtratoRamalResult.aspx?ramal={{x.RAMAL}}&codigo_usuario={{x.CODIGO_USUARIO}}&mes={{ dtvencimento.Valor | limitTo : 2 : 0}}&ano={{ dtvencimento.Valor | limitTo : 4 : 3}}&tipo=HTML" target="_blank"> {{x.RAMAL}}</a></td>                                   
                                   <td>{{x.CHAMADAS | number : 0}}</td>
                                   <td>{{x.MINUTAGEM | number : 2}}</td>
                                     <% If MostraCreditoRamal %>
                                        <td>{{x.CREDITO | currency }}</td>  
                                    <% end if %>     
                                   <td>{{x.MODELO}}</td>                                                                               
                                   <td>{{x.SERVICO | currency }}</td>                                                                               
                                   <td>{{x.CUSTO_RAMAL | currency }}</td>                                           
    ...
    <script>
        ...
        window.onload = function () {
            ...
            setTimeout(function () {                
                ...
                angular.element(document.getElementById('myApp')).scope().GetDatas();
        ...
        function CarregaGraficos()
        {
            ...
            //alert(codigousuario);
            $("#divLoader").show();
            $("#divGraficoMedia").load("uc_graficoUsuarioMedia.aspx?ramal=1&codigousuario=" + codigousuario + "&mostraArea=" + mostraArea, function () {
            ...
            //divGraficoServ
            $("#divGraficoServ").load("uc_GraficoUsuarioServicos.aspx?ramal=1&codigousuario=" + codigousuario, function () {
                //alert("Load was performed.");
                //$("#divLoader").hide();
                $("#divGraficoServ").show();
            });
